...
- Express 4.x + cors
- Vitest 4.x + fast-check 4.x + supertest 7.x (testes)
- ESLint 10.x + Prettier 3.x (qualidade de código)
...
| 8 | Subagents | Agentes especializados | Delegação de tarefas a subagentes customizados | [docs/kiro-subagents](docs/kiro-subagents/) |
...
│   ├── database/
│   │   └── products.ts         # Produtos em memória + interface Product
│   ├── routes/
│   │   ├── health.ts           # GET /health
│   │   └── products.ts         # GET /products (pipeline: validate → filter → sort → paginate)
│   ├── services/
│   │   └── productService.ts   # Lógica de negócio (validação, filtro, ordenação, paginação)
│   └── types/
...
├── .kiro/
│   ├── steering/               # Steering documents (tech, structure, product, testes, git)
│   ├── hooks/                  # Agent hooks configurados
│   ├── specs/                  # Specs (requirements, design, tasks)
│   ├── skills/                 # Skills customizadas (caveman, sql-optimization, TDD)
...
| Arquivo | Propósito |
|---------|-----------|
| `tech.md` | Stack, build system, convenções TypeScript |
| `structure.md` | Arquitetura, pastas, naming conventions |
| `product.md` | Domínio do produto, endpoints, features |
| `padroes-testes.md` | Padrões para escrita de testes |
| `guia-git-flow.md` | Fluxo de branches e PRs |
| `guia-commit.md` | Convenção de commits |
| `guia-trello.md` | Integração com Trello |

### Skills (`.kiro/skills/`)

| Skill | Descrição |
|-------|-----------|
| `caveman` | Comunicação ultra-compacta (economia de 65% de tokens) |
| `sql-optimization` | Otimização de queries SQL |
| `test-driven-development` | Workflow TDD (teste primeiro, depois implementação) |
